...
const TikTokEmbed: React.FC<TikTokEmbedProps> = ({ url, title, videoUrl, className = '' }) => {
    ...
    if (videoUrl && !hasError) {
        return (
            <div className={`relative w-full overflow-hidden shadow-lg aspect-[9/16] max-h-[650px] bg-black group rounded-2xl ${className}`}>
                <video
                    ref={videoRef}
                    src={videoUrl}
    ...
    return (
        <div className={`relative w-full overflow-hidden shadow-lg aspect-[9/16] max-h-[650px] bg-white group rounded-2xl ${className}`}>
            <iframe
